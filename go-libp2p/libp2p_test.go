...
func TestTransportConstructorTCP(t *testing.T) {
	...
	require.NoError(t, h.Network().Listen(a))
	b, _ := ma.StringCast("/ip4/127.0.0.1/udp/0/quic-v1")
	err = h.Network().Listen(b)
	require.Error(t, err)
...
func TestTransportConstructorQUIC(t *testing.T) {
	...
	require.NoError(t, h.Network().Listen(a))
	b, _ := ma.StringCast("/ip4/127.0.0.1/tcp/0")
	err = h.Network().Listen(b)
	require.Error(t, err)
	require.Contains(t, err.Error(), swarm.ErrNoTransport.Error())
}
...
func TestHostAddrsFactoryAddsCerthashes(t *testing.T) {
	addr, _ := ma.StringCast("/ip4/1.2.3.4/udp/1/quic-v1/webtransport")
	h, err := New(
...
func TestCircuitBehindWSS(t *testing.T) {
	...
	asni, _ := ma.StringCast(relayAddrWithSNIString)
	relayAddrWithSNI := []ma.Multiaddr{asni}
...
func TestCustomTCPDialer(t *testing.T) {
	...
	require.NoError(t, err)
	a, _ := ma.StringCast("/ip4/1.2.3.4/tcp/4")
