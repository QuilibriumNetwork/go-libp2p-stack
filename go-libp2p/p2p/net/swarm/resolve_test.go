...
func TestSwarmResolver(t *testing.T) {
	...
	res, err := swarmResolver.ResolveDNSComponent(ctx, tStringCast("/dns/example.com"), 10)
	require.NoError(t, err)
	require.Equal(t, 1, len(res))
	require.Equal(t, "/ip4/127.0.0.1", res[0].String())

	res, err = swarmResolver.ResolveDNSAddr(ctx, "", tStringCast("/dnsaddr/example.com"), 1, 10)
	require.NoError(t, err)
	require.Equal(t, 1, len(res))
	require.Equal(t, "/ip4/127.0.0.1", res[0].String())
	...
	t.Run("Test Recursive Limits", func(t *testing.T) {
		...
		res, err = swarmResolver.ResolveDNSAddr(ctx, "", tStringCast("/dnsaddr/0.example.com"), 256, 10)
		require.NoError(t, err)
		require.Equal(t, 1, len(res))
		require.Equal(t, "/ip4/127.0.0.1", res[0].String())

		res, err = swarmResolver.ResolveDNSAddr(ctx, "", tStringCast("/dnsaddr/0.example.com"), 255, 10)
		require.NoError(t, err)
		require.Equal(t, 1, len(res))
	...
	t.Run("Test Resolve at output limit", func(t *testing.T) {
		...
		res, err = swarmResolver.ResolveDNSAddr(ctx, "", tStringCast("/dnsaddr/example.com"), 256, 10)
		require.NoError(t, err)
		require.Equal(t, 10, len(res))
		for _, r := range res {
