...
func (a *addrsManager) getAddrs(localAddrs []ma.Multiaddr, relayAddrs []ma.Multiaddr) []ma.Multiaddr {
	...
	if rch != nil && *rch == network.ReachabilityPrivate {
		...
		if len(relayAddrs) > 0 {
			addrs = slices.DeleteFunc(addrs, func(a ma.Multiaddr) bool { isPubAddr, err := manet.IsPublicAddr(a); return isPubAddr || err != nil })
...
func (a *addrsManager) HolePunchAddrs() []ma.Multiaddr {
	...
	return slices.DeleteFunc(addrs, func(a ma.Multiaddr) bool { isPubAddr, err := manet.IsPublicAddr(a); return !isPubAddr || err != nil })
}
...
var p2pCircuitAddr, _ = ma.StringCast("/p2p-circuit")
