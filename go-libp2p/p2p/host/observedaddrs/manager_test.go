...
func TestObservedAddrsManager(t *testing.T) {
	...
	getConns := func(t *testing.T, n int, protocolCode int) []*mockConn {
		...
		protoPartMap := map[int]ma.Multiaddr{
			ma.P_TCP:          tStringCast("/tcp/1"),
			ma.P_QUIC_V1:      tStringCast("/udp/1/quic-v1"),
			ma.P_WEBTRANSPORT: tStringCast("/udp/1/quic-v1/webtransport"),
		}
		...
		for i := 0; i < n; i++ {
			ipPart := tStringCast(fmt.Sprintf("/ip4/1.2.3.%d", i))
	...
	t.Run("many observed addrs output size limited", func(t *testing.T) {
		...
		for i := 0; i < len(observedAddrs); i++ {
			observedAddrs[i] = tStringCast(fmt.Sprintf("/ip4/2.2.2.%d/tcp/2", i))
		}
		for i, c := range conns {
	...
	t.Run("WebTransport inferred from QUIC", func(t *testing.T) {
		...
		observedWebTransport := tStringCast("/ip4/2.2.2.2/udp/2/quic-v1/webtransport/certhash/uEgNmb28")
	...
	t.Run("WebTransport inferred from QUIC, with no WebTransport connections", func(t *testing.T) {
		...
		defer o.Close()
		observedQuic := tStringCast("/ip4/2.2.2.2/udp/2/quic-v1")
		inferredWebTransport := tStringCast("/ip4/2.2.2.2/udp/2/quic-v1/webtransport/certhash/uEgNmb28")
		c1 := newConn(quic4ListenAddr, tStringCast("/ip4/1.2.3.1/udp/1/quic-v1"))
		c2 := newConn(quic4ListenAddr, tStringCast("/ip4/1.2.3.2/udp/1/quic-v1"))
		c3 := newConn(quic4ListenAddr, tStringCast("/ip4/1.2.3.3/udp/1/quic-v1"))
		c4 := newConn(quic4ListenAddr, tStringCast("/ip4/1.2.3.4/udp/1/quic-v1"))
		o.maybeRecordObservation(c1, observedQuic)
		o.maybeRecordObservation(c2, observedQuic)
	...
	t.Run("SameObservers", func(t *testing.T) {
		...
		for i := 0; i < N; i++ {
			...
			ob2[i] = newConn(quic4ListenAddr, tStringCast(fmt.Sprintf("/ip4/1.2.3.%d/udp/2/quic-v1", i)))
		}
		for i := 0; i < N-1; i++ {
	...
	t.Run("SameObserversDifferentAddrs", func(t *testing.T) {
		...
		inferredWebTransport1 := tStringCast("/ip4/2.2.2.2/udp/2/quic-v1/webtransport/certhash/uEgNmb28")
		inferredWebTransport2 := tStringCast("/ip4/2.2.2.2/udp/3/quic-v1/webtransport/certhash/uEgNmb28")
	...
	t.Run("Old observations discarded", func(t *testing.T) {
		...
		c3 := newConn(webTransport4ListenAddr, tStringCast("/ip4/1.2.3.3/udp/1/quic-v1/webtransport"))
		c4 := newConn(webTransport4ListenAddr, tStringCast("/ip4/1.2.3.4/udp/1/quic-v1/webtransport"))
		c5 := newConn(quic4ListenAddr, tStringCast("/ip4/1.2.3.5/udp/1/quic-v1"))
		c6 := newConn(quic4ListenAddr, tStringCast("/ip4/1.2.3.6/udp/1/quic-v1"))
		...
		for i := 0; i < 10; i++ {
			...
			observedWebTransportWithCertHash = tStringCast(fmt.Sprintf("/ip4/2.2.2.%d/udp/2/quic-v1/webtransport/certhash/uEgNmb28", i))
			o.maybeRecordObservation(c1, observedQuic)
			o.maybeRecordObservation(c2, observedQuic)
			o.maybeRecordObservation(c3, observedWebTransport)
			o.maybeRecordObservation(c4, observedWebTransport)
	...
	t.Run("WebTransport certhash", func(t *testing.T) {
		...
		observedWebTransportWithCerthash := tStringCast("/ip4/2.2.2.2/udp/1/quic-v1/webtransport/certhash/uEgNmb28")
		inferredQUIC := tStringCast("/ip4/2.2.2.2/udp/1/quic-v1")
	...
	t.Run("Many connection many observations IP4 And IP6", func(t *testing.T) {
		...
		for i, idx := 0, 0; i < maxExternalThinWaistAddrsPerLocalAddr; i++ {
			...
			resWebTransportWithCertHashAddrs = append(resWebTransportWithCertHashAddrs, tStringCast(fmt.Sprintf("/ip4/2.2.%d.4/udp/2/quic-v1/webtransport/certhash/uEgNmb28", 9-i)))
			...
			resWebTransportWithCertHashAddrs = append(resWebTransportWithCertHashAddrs, tStringCast(fmt.Sprintf("/ip6/20%02x::04/udp/2/quic-v1/webtransport/certhash/uEgNmb28", 9-i)))
