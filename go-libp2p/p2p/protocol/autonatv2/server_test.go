...
func TestServerInvalidAddrsRejected(t *testing.T) {
	...
	t.Run("black holed addr", func(t *testing.T) {
		...
		res, err := c.GetReachability(context.Background(),
			[]Request{{
				Addr:         tStringCast("/ip4/1.2.3.4/udp/1234/quic-v1"),
				SendDialData: true,
			}})
		require.NoError(t, err)
		require.Equal(t, Result{AllAddrsRefused: true}, res)
	...
	t.Run("relay addrs", func(t *testing.T) {
		...
		res, err := c.GetReachability(context.Background(), newTestRequests(
			[]ma.Multiaddr{tStringCast(fmt.Sprintf("/ip4/1.2.3.4/tcp/1/p2p/%s/p2p-circuit/p2p/%s", c.host.ID(), c.srv.dialerHost.ID()))}, true))
		require.NoError(t, err)
		require.Equal(t, Result{AllAddrsRefused: true}, res)
	})
	...
	t.Run("too many address", func(t *testing.T) {
		...
		for i := 0; i < 100; i++ {
			addrs = append(addrs, tStringCast(fmt.Sprintf("/ip4/127.0.0.1/tcp/%d", 2000+i)))
		}
...
func TestServerDial(t *testing.T) {
	...
	unreachableAddr := tStringCast("/ip4/1.2.3.4/tcp/2")
...
func TestServerDataRequestWithAmplificationAttackPrevention(t *testing.T) {
	...
	err := c.host.Network().Listen(tStringCast("/ip6/::1/udp/0/quic-v1"))
	if err != nil {
...
func TestDefaultAmplificationAttackPrevention(t *testing.T) {
	q1 := tStringCast("/ip4/1.2.3.4/udp/1234/quic-v1")
	q2 := tStringCast("/ip4/1.2.3.4/udp/1235/quic-v1")
	t1 := tStringCast("/ip4/1.2.3.4/tcp/1234")
	...
	t2 := tStringCast("/ip4/1.1.1.1/tcp/1235") // different IP
	require.True(t, amplificationAttackPrevention(q2, t2))

	// always ask dial data for dns addrs
	d1 := tStringCast("/dns/localhost/udp/1/quic-v1")
	d2 := tStringCast("/dnsaddr/libp2p.io/tcp/1")
