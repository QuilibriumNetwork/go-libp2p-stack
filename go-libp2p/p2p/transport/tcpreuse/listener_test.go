...
func TestListenerSingle(t *testing.T) {
	listenAddr := tStringCast("/ip4/0.0.0.0/tcp/0")
	const N = 64
...
func TestListenerClose(t *testing.T) {
	...
	listenAddrs := []ma.Multiaddr{tStringCast("/ip4/0.0.0.0/tcp/0"), tStringCast("/ip6/::/tcp/0")}
...
func TestHitTimeout(t *testing.T) {
	...
	listenAddr := tStringCast("/ip4/127.0.0.1/tcp/0")
	ml, err := cm.DemultiplexedListen(listenAddr, DemultiplexedConnType_MultistreamSelect)
	require.NoError(t, err)
	defer ml.Close()
