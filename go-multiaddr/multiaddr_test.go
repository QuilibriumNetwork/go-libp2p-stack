...
func TestReturnsNilOnEmpty(t *testing.T) {
	...
	a, _ = StringCast("/ip4/1.2.3.4/tcp/1234")
	_, a = SplitFirst(a)
	_, a = SplitFirst(a)
	require.Nil(t, a)
	_, a = SplitFirst(a)
	...
	a, _ = StringCast("/ip4/1.2.3.4/tcp/1234")
	b, _ := StringCast("/tcp/1234")
	a = a.Decapsulate(b)
	b, _ = StringCast("/ip4/1.2.3.4")
	a = a.Decapsulate(b)
	require.Nil(t, a)
...
func TestDecapsulate(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run("Decapsulating with components"+tc.left, func(t *testing.T) {
			l, _ := StringCast(tc.left)
			left, last := SplitLast(l)
